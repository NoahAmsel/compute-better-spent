...
for attn in "\"QK=LowRankQK(heads,dim,dim//heads,attn_mult=attn_mult);VO=LowRankVO(heads,dim,dim//heads,use_bias=use_bias)\""; do  # "none"
        # --partition=a100_1 \
        # --gres=gpu:a100:1 \
sbatch  --job-name=muP_test \
        --nodes=1 \
        --ntasks-per-node=1 \
        --cpus-per-task=4 \
        --gres=gpu:1 \
...
--run_name=muP_test_noQKnorm/${scale_factor}x/${lr}/${attn} \
--dataset=${ds} \
